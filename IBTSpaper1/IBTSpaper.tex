...
In this section we applied the percentile bootstrap and bias-corrected bootstrap methods for estimating variability in abundance indices. For both of these methods we applied the stratified procedure described in Section \ref{sec:datrasstratifiedbootstrap}. We also give abundance indices from the three ALK methods described in Section \ref{sec:alkmethods}.
...
\clearpage
%coverage error sqrt{0.95*0.05/1000} 
Table \ref{confidencelevels} gives the coverage probability (CP), which is the percentage of time the true value lies within the interval for the bias-corrected and percentile bootstrap methods for 1000 bootstrap replications. The mean width of the confidence intervals is also given. The standard error estimate of the coverage interval is $0.00689$. \\
...
\caption{Coverage probability (CP), which is the proportion of time the true value lies within the interval, and the mean width of the confidence intervals from the bias-corrected bootstrap and percentile bootstrap methods are given for each of three ALK methods. The nomial value is $0.95$} \label{confidencelevels}}
\begin{tabular}{ccccccccccccccccccccccccccc}
\hline \\[0.1ex]
{Species}&{ $a$ } & \multicolumn{6}{c}{\bf Bias-corrected}  && \multicolumn{6}{c}{\bf Percentile } \\ [1.1ex]
 & & \multicolumn{2}{c}{DATRAS } & \multicolumn{2}{c}{\thead{ Haul based }} & \multicolumn{2}{c}{\thead{ Model based}} &&  \multicolumn{2}{c}{DATRAS } & \multicolumn{2}{c}{\thead{ Haul based }} & \multicolumn{2}{c}{\thead{ Model based}} \\[1.5ex]
 % & \multicolumn{3}{c}{DATRAS Bootstrap}   & \multicolumn{3}{c}{Stratified Bootstrap}   & \multicolumn{3}{c}{Stratified Bootstrap} \\[1.5ex]
   \cmidrule(lr{0.5em}){3-8}  \cmidrule(lr{0.1em}){10-15}   \\ [0.1ex]
& & CP & mean width & CP & mean width   & CP & mean width  && CP &  mean width & CP &  mean width     & CP & mean width \\[0.5ex]
\cmidrule(lr{0.5em}){3-4}  \cmidrule(lr{0.5em}){5-6}  \cmidrule(lr{0.5em}){7-8} \cmidrule(lr{0.5em}){10-11}  \cmidrule(lr{0.5em}){12-13}  \cmidrule(lr{0.5em}){14-15}\\ [0.1ex]
%\hline\\[1.0ex]
...
%As discussed in Section \ref{sec:datrasstratifiedbootstrap} the naive bootstrap procedure proposed by DATRAS to estimate uncertainty around indices of abundance is not appropriate for IBTS survey. Firstly, the DATRAS bootstrap procedure ignores the fine-scale stratification in the sampling process, which would lead to an overestimation of the uncertainty. Secondly, it ignores the sampling procedure of age-length data collected at the haul level, which would lead to an underestimation of the uncertainty. So there are biases in both directions, which are difficult to quantify. To illustrate the implications of DATRAS bootstrap procedure, we compare estimates from this bootstrap procedure with our stratified bootstrap procedure and the results show that an overestimation of the uncertainty around indices of abundance is overwhelming, suggesting that it is highly relevant to account for the variation in the data over large areas (Table \ref{resultstablecodandsaitheAppendix} in Web appendix \ref{secAp:resultsdatrasALK} presents these results for year 2017 Q3).   
