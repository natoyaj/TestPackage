...
Recall that the main assumption of DATRAS ALK is that the age-length compositions of species across large areas are the same. Our proposed nonparametric haul-based ALK method and spatial ALK model accounts for the spatial distribution of the ages given length 

\begin{itemize}
\item generate plots for year 2018 Q1. Possibly show same age given length group for both species?
\item discuss evidence against the null hypothesis of no spatial effect in ALK
\item discuss differences in estimates from ALK methods (Table \ref{resultstablecodandsaithe} - in terms of relative standard error estimates; overestimation and underestiamtion from DATRAS meethod not knowing the strength in either direction)
...
& 1  & 0.764  & 0.26 ($34 \%$) & 0.31 & 1.33 & 0.60  & 0.24 ($40 \%$) & & & 0.70 & 0.36 ($51 \%$) \\[1ex]
...
{\bf Species} &{\bf $a$ } & \multicolumn{2}{c}{\bf Full data}  & \multicolumn{6}{c}{\thead{\bf Reduced data by $2$ cm}} & \multicolumn{6}{c}{\thead{\bf Reduced data by $5$ cm}} \\[1.5ex]
 ...
 % & \multicolumn{3}{c}{DATRAS Bootstrap}   & \multicolumn{3}{c}{Stratified Bootstrap}   & \multicolumn{3}{c}{Stratified Bootstrap} \\[1.5ex]
%   \cmidrule(lr{0.5em}){2-5}  \cmidrule(lr{0.1em}){6-9} \cmidrule(lr{0.5em}){10-13}  \\ [0.1ex]
& &$\widehat{mCPUE_{N,a}}$ & $\widehat{SE}_{mCPUE_{N,a}}$   & Estimate  & LB & UB   & Estimate &  LB & UB & Estimate  & LB & UB   & Estimate &  LB & UB &    \\[0.5ex]
\cmidrule(lr{0.5em}){3-4}  \cmidrule(lr{0.5em}){5-10} \cmidrule(lr{0.5em}){11-16}  \\ [0.1ex]
%\hline\\[1.0ex]
\raisebox{1.5ex}{\bf cod} & 0  & 0 & 0   &  0   &  0  & & & 0 & 0  \\[1ex]
& 1  & 0.764  & 0.26  &  0.60  & 0.24  & & & 0.70 & 0.36  \\[1ex]
