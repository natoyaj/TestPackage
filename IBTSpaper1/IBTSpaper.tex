...
\centering
\begin{tabular}{@{}ccc@{}}
\subfloat[Map of IBTS Q1]{\includegraphics[width=0.45\textwidth]{icesroundfishmap.jpg}} & 
\subfloat[Map of IBTS Q3]{\includegraphics[width=0.45\textwidth]{icesroundfishmap.jpg}} &
\end{tabular}
\caption[]{Standard roundfish areas used for roundfish since 1980, for all standard species since 1991. Additional RFA 10 added in 2009. For example, the number 1 indicates ICES Index Area 1, and an ICES Statitical rectangle (ST) in IA 1 is 43F1 \citep{ICES2015}.}
\label{icesroufismap}
\end{figure*} 
...
In this section we describe how the uncertainty of the CPUE estimates are calculated. We use nonparametric bootstrapping to quantify the uncertainty of the CPUEs. In nonparametric bootstrapping independent samples of lengths and age are drawn with replacement from the original data and approximate $95\%$ confidence intervals are obtained using the percentile method  and bias-corrected percentile method  \citep{carpenter2000bootstrap}. Nonparametric resampling allows us to estimate the sampling distribution of the CPUE empirically without making assumptions concerning the data.

%\emph{\large Percentile Method}. The percentile method makes direct use of the empirical sampling distribution, assuming that the statistic from each bootstrap sample is unbiased and symmetric, but not necessarily normal \citep{carpenter2000bootstrap}.  The $100(\alpha /2)$ and $100(1- \alpha /2)$ percentile values of the bootstrap sampling distribution estimate are used as the upper and lower confidence limits for the density $\rho$. The avoidance of the assumption of normality and its simplicity in implementation are the main attractions of this method. However, the coverage error is often substantial if the distribution of the estimated parameter of interest is not nearly symmetric. Also, at least 1000 bootstrap samples are required to obtain sufficiently accurate $95\%$ confidence intervals \citep{briggs1999constructing, carpenter2000bootstrap}.

%\emph{\large Biased-corrected}. The Bias-Corrected method is an alternative to the Percentile method, which seeks to adjust for the bias and skew of the sampling distribution. This revised method assumes that there is a montonic increasing function and the estimator $\hat{\rho}$ has a monotonic increasing function $f()$ such that the transformed values $f(\hat{\rho})$ are normally distributed with mean $f(\rho) - z_{0}$ and standard deviation one, where $z_{0}$ are the standard normal limits \citep{puth2015variety}.
...
For illustration we show the implications of using DATRAS bootstrap procedure for estimating the uncertainty around indices of abundance  in Table \ref{resultstablecodandsaitheAppendix} in Web appendix \ref{secAp:resultsdatrasALK}. Compared with the stratified bootstrap procedure, DATRAS bootstrap procedure gives an overestimation of the uncertainty for all age groups, suggesting that it is highly relevant to account for the variation in the data over large areas. 
...
\centering
\begin{tabular}{@{}ccc@{}}
\subfloat[Bias-corrected bootstrap method]{\includegraphics[width=0.95\textwidth]{cod2017Q3BiasCorr.jpeg}} & \\
\subfloat[Percentile bootstrap method]{\includegraphics[width=0.95\textwidth]{cod2017Q3Percentile.jpeg}} &
\end{tabular}
\caption[]{Estimated confidence intervals ($\mathrm{CI}_{96}$) from bias-corrected and percentile bootstrap methods for cod in year 2017 Q3. Estimated indices of abundance (Estimate), and its standard error (SE(Estimate), bootstrap mean (mean) and median estimates are also given.}
...
\centering
\begin{tabular}{@{}ccc@{}}
\subfloat[Bias-corrected bootstrap method]{\includegraphics[width=0.95\textwidth]{saithe2017Q3BiasCorr.jpeg}} & \\
\subfloat[Percentile bootstrap method]{\includegraphics[width=0.95\textwidth]{saithe2017Q3Percentile.jpeg}} &
\end{tabular}
\caption[]{Estimated confidence intervals ($\mathrm{CI}_{96}$)  from bias-corrected and percentile bootstrap methods for saithe in year 2017 Q3. Estimated indices of abundance (Estimate), and its standard error (SE(Estimate)), bootstrap mean (mean) and median estimates are also given.}
\label{percentileBiascorrectedCIsaithe}
\end{figure*} 


\clearpage
%\subsection{Comparison of ALK methods}
...
\centering
\begin{tabular}{@{}ccc@{}}
\subfloat[age-length distribution of cod]{\includegraphics[width=0.45\textwidth]{cod2015Q1.jpeg}} & 
\subfloat[age-length distribution of saithe]{\includegraphics[width=0.45\textwidth]{saithe2015Q3.jpeg}} &
\end{tabular}
...
\caption{Estimates of abundance indices and standard error estimates for the North Sea cod and saithe species when all the otolith data is used and when one otolith per 2 cm or 5 cm length group is used. The lower bounds (LB) and upper bounds (UP) of approximate $95\%$ confidence intervals  are given.}\label{resultsoptimum}}
\begin{tabular}{ccccccccccccccccccccccccccc}
\hline \\[0.1ex]
...
% \cmidrule(lr{0.5em}){12-13}   \cmidrule(lr{0.5em}){14-15}\\ [0.1ex]
%\hline\\[1.0ex]
...
%\captionsetup{font=small, width = 16.5cm}{
%\caption{Estimated abundance index $\hat{\lambda}_{a}$, lower bound (LB) and upper bound (UP) of approximate 95\% confidence intervals from the percentile and bias-corrected methods for each of three ALK methods for 200 bootstrap replication. The width of the confidence intervals are given.} \label{confidencelevels}}
%\begin{tabular}{ccccccccccccccccccccccccccc}
%\hline \\[0.1ex]
%{Species}&{ $a$ } & \multicolumn{12}{c}{\bf Bias-corrected}  & \multicolumn{12}{c}{\bf Percentile } \\ [1.1ex]
% & & \multicolumn{4}{c}{DATRAS } & \multicolumn{4}{c}{\thead{ Haul based }} & \multicolumn{4}{c}{\thead{ Model based}} &  \multicolumn{4}{c}{DATRAS } & \multicolumn{4}{c}{\thead{ Haul based }} & \multicolumn{4}{c}{\thead{ Model based}} \\[1.5ex]
% % & \multicolumn{3}{c}{DATRAS Bootstrap}   & \multicolumn{3}{c}{Stratified Bootstrap}   & \multicolumn{3}{c}{Stratified Bootstrap} \\[1.5ex]
%   \cmidrule(lr{0.5em}){3-14}  \cmidrule(lr{0.1em}){15-26}  \\ [0.1ex]
%& &$\hat{\lambda}_{a}$ & LB&  UB & width &$\hat{\lambda}_{a}$ & LB&  UB & width  &$\hat{\lambda}_{a}$ & LB&  UB & width  & $\hat{\lambda}_{a}$ & LB&  UB & width& $\hat{\lambda}_{a}$ & LB&  UB & width    & $\hat{\lambda}_{a}$ & LB&  UB & width \\[0.5ex]
%\cmidrule(lr{0.5em}){3-6}  \cmidrule(lr{0.5em}){7-10}  \cmidrule(lr{0.5em}){11-14} \cmidrule(lr{0.5em}){15-18}  \cmidrule(lr{0.5em}){19-22} \cmidrule(lr{0.5em}){23-26}  \\ [0.1ex]
%%\hline\\[1.0ex]
%
%{\bf cod} \\[1.0ex]
%\cmidrule(lr{0.5em}){1-1}\\[1.0ex]
%
%\raisebox{1.5ex}{2017 Q3}&   &  &  &  &  &   \\[1ex]
%& 1  & &  & & &     \\[1ex]
%& 2  & &  & & &    \\[1ex]
%& 3  & &  & & &  \\[1ex]
%& 4  & &  & & &    \\[1ex]
