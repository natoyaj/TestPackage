...
& Cod with missing age in RFAs     & 0.30\%& 0.13\% & 0.13\% & 0.34\% & 0.68\% &  0.07\% & 0.51\%  & 0.11\%  \\[1.5ex]  
& Cod with missing age in hauls    & 10.06\% & 14.20\% & 6.18\%& 6.32\% & 2.25\% &0.27\%   & 1.40\%   & 0.77\% \\[2.5ex] 
...
& Saithe with missing age in RFAs      &1.2\% & 0\%		& 0\%  & 0.03\% &0.14\%  &  0.05\% & 0.03\%  & 0\%  \\[1.5ex]  
...
\end{figure*} 
 ...
 In this section we compare the ALK procedures by investigating how the estimated SSB differs in an assessment study. We use the assessment model SAM \citep{nielsen2014estimation}, which is currently used in assessment for both the species in our case study. 
The default configurations in SAM are used. %We estimate the assessment model using indices calculated with area based ALK and hauls based ALKs. 
The estimated SSBs with usage of the two different ALKs are illustrated in figure \ref{fig:assessment} for both cod and saithe. We see that the differences between the estimated SSB with the two ALK procedures are small. Note that only the point estimates of the indices are used in the assessment study. Data needed for assessment, including the year range for the indices, are extracted from \href{www.stockassessment.org}{www.stockassessment.org}.  \olav{(we should include the CV in the assessment modelling, and use the same configurations as in the real assessment.)} 
...
\caption[]{Estimated SSB with use of indices calculated with area based and haul based ALK, with 95\% confidence intervals. The dotted lines and light blue intervals refers to assessment with use the haul based ALK. The dark line and grey interval refer to assessment with use of the area based ALK. }
...
In this section we investigate the effect of reducing effort with respect to otolith collection. The details of how the reduction is acheived is given in section \ref{sec:reducingeffort}.  We illustrate the effect by investigating how the CV of the mCPUE (\ref{eq:abundanceestimatornorthsea}) changes, and by investigating how the SSB changes in an assessment study. 

\subsection{Effect on CV}
Figure \ref{fig:removalCodQ1} illustrates the CV with several selections of length groups in year 2015-2018 Q1.  We see that the CV typically increases as length group width increases. This increase seems to be particularly strong for cod in age range 3-5 years. Compared to the original CV, we see from figure \ref{fig:removalCodQ1} that the CV is almost unchanged when using twice as wide length groups below 40cm and the original length groups above 40cm.  At the same time, the number of sampled otoliths is typically reduces with approximately 30\% with that length groups selection. This observation gives a strong case for that otolith sampling from short fish can be reduces with a minor effect on the mCPUE.
