...
\end{figure*} 
 ...
 In this section we compare the ALK procedures by investigating how the estimated SSB changes in an assessment study. We use the assessment model SAM \citep{nielsen2014estimation}, which is currently used in assessment for both the species in our case study. %The configurations in SAM are selected as the same as currently used in assessment, see \href{www.stockassessment.org}{www.stockassessment.org} for details. 
The default configurations in SAM are used. The SSB is illustrated in figure \ref{fig:assessment} for both cod and saithe. We see that the differences in assessment with the two ALK procedures are small. Note that only the point estimates of the indices are used in the assessment study. \olav{(we should include the CV in the assessment modelling, and use the same configurations as in assessment.)}
