...
\item Sample age observations from the resampled hauls obtained in step 1. For the DATRAS procedure this is achieved by (a), and for the stratified procedure this is achieved by (b):
\begin{enumerate}
...
& $\% \mathrm{length \ groups}$ with missing ages in RFA      &  &   &   &   \\[1.5ex]  
& $\% \mathrm{length \ groups}$ with missing ages in haul     &  &   &   &   \\[0.5ex]
...
\captionsetup{font=small, width = 14.5cm}{
 \caption{Estimated proportion of age 1 and 2 year-old Gadus morhua (cod) of length 20 cm long in Skagerak. The green circles $(\mycirc[green])$ and blue circles $(\mycirc[blue])$ are observations of one and two year old cod, respectively, which are in the length interval 19 cm to 21 cm. }\label{fig:proportionSkagerak}}
\end{figure}

\nat{Please look at the plots below for the different ages given different lengths; see Figure \ref{fig:proportionLengthsAppendix}. I am a bit confused with the colours of the different ages. Please clearly explain which colour is associated with what age. The codes say age 1 = black, age 2 = green, age 3 = blue, but it's somewhat confusing on the plots (see Figure \ref{fig:proportionSkagerak} above for example, which shows that for length 20 cm the points are for ages 2 and 3 based on the colours, but what I plotted is ages 1 and 2 given length 20 cm). Also, I have not noticed in the codes any colour associated with age 0. There is also the graph showing the probability of age given length 20 cm for years 2017-2018 in Q1 and Q3; see Figure \ref{fig:proportionYearsAppendix}.  These plots will go in the appendix.}
...
\subfloat[]{\includegraphics[scale=0.4]{figures/spatialLength40Age4Quarter1codYear2018.jpeg}}
\quad
 \subfloat[]{\includegraphics[scale=0.4]{figures/spatialLength50Age4Quarter1codYear2018.jpeg}}
\subfloat[]{\includegraphics[scale=0.4]{figures/spatialLength50Age5Quarter1codYear2018.jpeg}}
\captionsetup{font=small, width = 14.5cm}{
 \caption{\nat{I am not sure about these colours for the different ages} }\label{fig:proportionLengthsAppendix}}
\end{figure}
...
%\clearpage
\subsection{Sufficient number of otoliths to sample for age determination}
\label{sec:optimumeffortresults}
In this section we investigate the effect of sampling fewer otoliths on estimated relative abundance at age and its uncertainty using the procedure described in Section \ref{sec:reducingeffort}. \nat{As mentioned before and confirmed by Jon Helge,  we should plot the percentage relative standard error $(uncertainty/estimate \times 100)$ for all the data versus the percentage relative standard for each length group: 1cm, 2cm, 3cm, 4cm or 5cm, for each age group for comparison.}
